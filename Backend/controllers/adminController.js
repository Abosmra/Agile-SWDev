...
module.exports = function setupAdminRoutes(app) {
  app.get('/api/admin/message-contacts', authenticate, requireRoles(['Admin']), async (req, res) => {
    try {
      const [staff, students, studentConversations] = await Promise.all([
        runQuery(
          req.app.locals.db,
          `SELECT StaffID AS id, Name AS name, Role AS role, Department AS department, ContactInfo AS email
           FROM Staff
           ORDER BY Role, Name`
        ),
        runQuery(
          req.app.locals.db,
          `SELECT UserID AS id, 
             COALESCE(NULLIF(TRIM(GivenName || ' ' || FamilyName), ''), Username) AS name,
             Role AS role, Username AS email
           FROM Users
           WHERE Role = 'Student'
           ORDER BY name`
        ),
        runQuery(
          req.app.locals.db,
          `SELECT
             CASE WHEN sender.Role = 'Student' THEN sender.UserID ELSE receiver.UserID END AS studentUserId,
             CASE
               WHEN sender.Role = 'Student'
                 THEN COALESCE(NULLIF(TRIM(sender.GivenName || ' ' || sender.FamilyName), ''), sender.Username)
               ELSE COALESCE(NULLIF(TRIM(receiver.GivenName || ' ' || receiver.FamilyName), ''), receiver.Username)
             END AS name,
             ...
             (SELECT Body
                ...
                WHERE latest.ToStaffID = m.ToStaffID
                  AND (
                    latest.FromUserID = CASE WHEN sender.Role = 'Student' THEN sender.UserID ELSE receiver.UserID END
                    OR latest.ToUserID = CASE WHEN sender.Role = 'Student' THEN sender.UserID ELSE receiver.UserID END
                  )
      ...
      res.json([
        ...staff.map((member) => ({
          ...
          lastSnippet: member.email || member.department
        })),
        ...students.map((student) => ({
          ...student,
          conversationType: 'student',
          lastSnippet: student.email || 'Student'
        })),
        ...studentConversations.map((conversation) => ({
          id: `student-${conversation.studentUserId}-${conversation.staffId}`,
          conversationType: 'student',
